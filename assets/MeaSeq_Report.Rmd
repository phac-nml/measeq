...
metrics_dt <- metrics_df %>%
  ...
  # Row Highlighting
  #  1 - pass, 2 - fail, 3 - partial, 4 - warning
  formatStyle(
      'status_colour_code',
      target = 'row',
...
summary_depth_df <- transormed_data %>%
  group_by(position) %>%
  summarise(
...
p <- ggplot(summary_depth_df, aes(x = position)) +
  ...
  labs(title = "Normalized median read depth",
       x = "Genome position", y = "Proportional read depth") +
  theme_classic() +
...
> **Figure 1.** Normalized median read depth and deviation. Normalized proportional read depth for each position in a sample was calculated using the maximum depth observed. All positions were summarized plotting the median read depth plus or minus 1 standard deviation surrounding the median.
