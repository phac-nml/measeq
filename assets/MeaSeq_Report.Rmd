...
for (i in 1:nrow(summary_df)) {
  ifelse(
      summary_df[[i,"genotype"]] != genotype |
      is.na(summary_df[[i,"genotype"]]) |
...
# Set empty Genotypes to NA for highlighting later
summary_df$Genotype[is.na(summary_df$Genotype)] <- "NA"
...
metrics_dt <- summary_df %>%
  ...
  # Highlight cells where the nextclade Genotype doesn't match the reference Genotype
  formatStyle(
    'Genotype',
    target = "cell",
    fontWeight = styleEqual(
      unique(summary_df$Genotype),
      ifelse(unique(summary_df$'Genotype') != genotype, "bold", "")
    ),
    `text-decoration` = styleEqual(
      unique(summary_df$Genotype),
      ifelse(unique(summary_df$'Genotype') != genotype, "underline", "")
    )
  ) %>%
