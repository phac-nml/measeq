...
> All positions with >=10 reads, a positional read base quality of >=20 and a map quality >=30 were included in the analysis.
> Mixed sites in this figure that are not in the variant table are variants that have failed processing so they are reference bases in the final consensus and not IUPAC bases.
...
if (file.exists(var_f)) {
  ...
  ddt <- variants %>%
    datatable(rownames = FALSE,
      caption = htmltools::tags$caption(
        style = 'caption-side: top',
        htmltools::strong('Table III: '), 'Consensus Variants Summary Table. All variants found in the consensus sequence are in the following table'
      ),
      options = list(
        pageLength = 10,
        scrollX = FALSE,
